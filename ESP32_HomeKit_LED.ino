 ...
 *   If not possible or WebSerial isn't needed, WebSerial can be removed
 *  You must disable USB serial input to power the ESP32 with 5V input and not a USB cable
 *   HomeSpan will otherwise get stuck in a loop looking for serial input that doesn't exist and not load
...
#define NUM_PIXELS             95                          // Number of pixels in LED strip
char* name =                   "L Office Credenza";        // Customize controller's name, will be shown in the serial output
bool disableUSBserial =        true;                       // Disables USB serial input, allowing HomeSpan to boot without USB cable
bool enableWebSerial =         true;                       // Enables all items for WebSerial
String hostname =              String(name);               // Used to change the device's hostname to reflect its name; e.g., name "Media Center" will become hostname "media-center"

#define NEOPIXEL_RGB_PIN       15                           // Data pin for LED strip
#define STA_PIN                2                            // Onboard status LED on ESP32
#define CTL_PIN                4                            // Control pin for physical button
...
void setup() {
  ...
  SPAN_ACCESSORY("Pixel RGB");
    new NeoPixel_RGB(NEOPIXEL_RGB_PIN,NUM_PIXELS);                      // Create NeoPixel RGB Strand with full color control
